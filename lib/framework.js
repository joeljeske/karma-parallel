...
const {createAggregateReporter} = require('./aggregated-reporter');
...
function setupCoverageReporters(injector, reporters) {
  ...
  reporters.forEach((reporter) => {
    if (/coverage|istanbul/i.test(reporter)) {
      const key = `reporter:${reporter}`;
      const aggregatedKey = `parallel-aggregate:${key}`;
      if (Object.hasOwnProperty.call(injector._providers, key)) {
        // TODO: Ensure this injector private API is stable "enough". Does it work with karma <1.0, <2.0?
        injector._providers[aggregatedKey] = injector._providers[key];
        injector._providers[key] = [injector.instantiate, createAggregateReporter(aggregatedKey), 'type'];
      }
    }
  });
}

module.exports = function(/* config */fullConfig, emitter, logger, injector) {
  ...
  setupCoverageReporters(injector, fullConfig.reporters);
