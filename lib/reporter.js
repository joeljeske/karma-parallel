...
const AggregatedCoverageReporter = function(injector, config, baseReporterDecorator, formatError) {
  ...
  const reporters = config.coverageReporters
    .map((reporter) => injector.createChild([locals], [`reporter:${reporter}`]).get(`reporter:${reporter}`));
  let aggregatedBrowserState;

  const callThrough = _.rest((name, args) => {
    reporters.forEach((reporter) => {
      if (_.isFunction(reporter[name])) {
        reporter[name].apply(reporter, args);
      }
    });
  });
...
AggregatedCoverageReporter.$inject = ['injector', 'config.parallelOptions', 'baseReporterDecorator', 'formatError'];
